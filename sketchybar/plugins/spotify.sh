...
update ()
{
  ...
  if [ $PLAYING -eq 0 ]; then
    if [ "$ARTIST" == "" ]; then
      args+=(--set spotify.name label="$TRACK  |  $ALBUM" drawing=on label.padding_right=15)
    else
      args+=(--set spotify.name label="$TRACK  |  $ARTIST" drawing=on label.padding_right=15)
    fi
    args+=(--set spotify.play icon=􀊆 \
           --set spotify.shuffle icon.highlight=$SHUFFLE \
           --set spotify.repeat icon.highlight=$REPEAT)
  else
    args+=(--set spotify.name label="Paused" label.padding_right=15 \
