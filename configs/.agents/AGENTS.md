...
- Use plain, direct language; avoid jargon when ordinary words will do
- Be concise by default; prefer short, direct sentences and lead with the answer or outcome
- Briefly explain necessary technical terms
- Go into depth when asked or when important context is needed to prevent confusion, mistakes, or risk
...
- If a newer user instruction conflicts with an earlier one, clarify
  - If the conflict is within AGENTS.md, suggest the conflict be reconciled
...
- **Make illegal states unrepresentable**; prefer algebraic data types/discriminated unions over boolean flags and loosely optional fields
...
- Keep abstractions consciously constrained, pragmatically parameterized, and documented when non-obvious
...
- Avoid over-engineering; do not add features, abstractions, configurability, dependencies, speculative handling, or refactors beyond what the task requires
...
- Fix behavior at its existing ownership point before introducing helpers, modules, types, files, or exports
...
- Do not create production seams merely to unit-test implementation details; test observable behavior through an existing seam
- Before adding a file or exported symbol, ask whether the behavior can remain clear and correct in the owning function
- When equivalent implementations exist, prefer fewer concepts, files, exports, and state transitions—not merely fewer lines
- Address material correctness, security, and reliability risks without letting optional improvements expand scope
- Mention potentially useful broader work briefly instead of implementing it by default
...
- If intent is unclear, confirm before acting
...
- Never bypass safeguards with destructive shortcuts - warn if explicitly requested and confirm
